  ...
  <Card className="bg-[#161b22] border-gray-700/50 hover:border-gray-600/60 transition-all duration-300 hover:shadow-lg group">
    <CardContent className="p-3 sm:p-4 lg:p-6">
      ...
      <div>
        ...
        <p className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-1 truncate">
          {value}
        </p>
        {subtitle && (
    ...
    <CardContent className="pt-0">
      <div className="space-y-3 sm:space-y-4">
        {metrics.map((metric, index) => (
          ...
          >
            <span className="text-xs sm:text-sm text-gray-400 truncate pr-2">
              {metric.label}
            </span>
      ...
      <div className="space-y-3 sm:space-y-4">
        <div className="space-y-2">
          ...
          <div className="w-full bg-[#0d1117] rounded-full h-2">
            <div
              className="bg-gradient-to-r from-[#58a6ff] to-[#0969da] h-2 rounded-full"
              style={{ width: "35%" }}
            ></div>
          </div>
...
export default function AccountSummary() {
  ...
  const fetchAccountSummary = async () => {
    try {
      setRefreshing(true);
      const response = await api.get("/trading/account/summary/");
  ...
  const portfolioValue = useMemo(() => {
    ...
    return (
      ...
      parseFloat(summary.margin) +
      parseFloat(summary.unrealized_pnl)
    );
  }, [summary]);

  const tradingMetrics = useMemo(() => {
    if (!summary) return [];
    const totalTrades = summary.history.length;

    // Calculate Win Rate
    let winningTrades = 0;
    const tradesByOrder = {};
    summary.history.forEach((trade) => {
      if (!tradesByOrder[trade.order]) {
        tradesByOrder[trade.order] = [];
      }
      tradesByOrder[trade.order].push(trade);
    });

    // This is a simplified win rate calculation. A more robust solution would
    // track P&L per closed position.
    Object.values(tradesByOrder).forEach((trades) => {
      const buys = trades.filter((t) => t.transaction_type === "BUY");
      const sells = trades.filter((t) => t.transaction_type === "SELL");
      if (buys.length > 0 && sells.length > 0) {
        const avgBuyPrice =
          buys.reduce((acc, t) => acc + parseFloat(t.executed_price), 0) /
          buys.length;
        const avgSellPrice =
          sells.reduce((acc, t) => acc + parseFloat(t.executed_price), 0) /
          sells.length;
    ...
    const avgTradeSize =
      totalTrades > 0
        ? summary.history.reduce(
            (acc, trade) =>
              acc + parseFloat(trade.executed_price) * trade.quantity,
            0
          ) / totalTrades
        : 0;

    return [
      { label: "Total Trades", value: totalTrades, trend: "up" },
      { label: "Win Rate", value: `${winRate.toFixed(2)}%`, trend: "up" },
      { label: "Avg Trade Size", value: formatCurrency(avgTradeSize) },
      { label: "Max Drawdown", value: "₹3,250", trend: "down" }, // Placeholder
    ];
  }, [summary]);

  const riskMetrics = [
    { label: "Portfolio Beta", value: "1.24" }, // Placeholder
    { label: "Sharpe Ratio", value: "1.89", trend: "up" }, // Placeholder
    { label: "VaR (1 day)", value: "₹8,500" }, // Placeholder
    { label: "Risk Score", value: "Medium" }, // Placeholder
  ];
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-24 sm:h-32 bg-[#161b22] rounded-lg animate-pulse"
            />
          ))}
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
          {[...Array(2)].map((_, i) => (
            <div
              key={i}
              className="h-48 sm:h-64 bg-[#161b22] rounded-lg animate-pulse"
            />
          ))}
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm lg:text-base text-gray-400">
              Virtual trading account status
            </p>
          </div>
        </div>
        <Button
        ...
        <Card className="bg-[#161b22] border-gray-700/50">
          ...
          <CardContent className="pt-0">
            <div className="space-y-3">
              {summary.history.slice(0, 3).map((trade, index) => (
                <div
                  key={index}
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-xs sm:text-sm font-medium text-gray-200 truncate">
                      {trade.transaction_type} Order Executed
                    </p>
                  ...
                  <span
                    className={`text-xs sm:text-sm font-semibold flex-shrink-0 ml-2 ${
                  ...
                  >
                    {trade.transaction_type === "BUY" ? "-" : "+"}
                    {formatCurrency(
                      parseFloat(trade.executed_price) * trade.quantity
                    )}
                  </span>
                </div>
              ))}
          ...
          <CardContent className="pt-0">
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-[#0969da]/20 text-[#58a6ff] border-[#0969da]/30 text-xs">
                  Active
                </Badge>
              </div>
              <div className="flex items-center justify-between">
