...
export const PARAM_CONFIG = {
  ...
  // Indicators
  SMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  EMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  WMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  HMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  ALMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  KAMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  DEMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  TEMA: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  RSI: [{ key: 'period', label: 'Period', default: 14 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  ROC: [{ key: 'period', label: 'Period', default: 9 }, { key: 'source', label: 'Source', type: 'source', default: 'close' }],
  CCI: [{ key: 'period', label: 'Period', default: 20 }],
  ADX: [
    { key: 'period', label: 'Period', default: 14 },
    { key: 'output_line', label: 'Output', type: 'select', options: ['ADX', '+DI', '-DI'], default: 'ADX' }
  ],
  DMI: [
    { key: 'period', label: 'Period', default: 14 },
    { key: 'output_line', label: 'Output', type: 'select', options: ['ADX', '+DI', '-DI'], default: 'ADX' }
  ...
  MACD: [
    ...
    { key: 'output_line', label: 'Output', type: 'select', options: ['MACD_LINE', 'MACD_SIGNAL', 'MACD_HISTOGRAM'], default: 'MACD_LINE' }
  ],
  BOLLINGER_BANDS: [
    { key: 'period', label: 'Period', default: 20 },
    { key: 'std_dev', label: 'StdDev', default: 2 },
    { key: 'output_line', label: 'Output Line', type: 'select', options: ['UPPER', 'MIDDLE', 'LOWER'], default: 'UPPER' }
  ],
  KELTNER_CHANNEL: [
    { key: 'period', label: 'Period', default: 20 },
  ...
  STOCHASTIC: [
    ...
    { key: 'output_line', label: 'Output Line', type: 'select', options: ['K', 'D'], default: 'K' }
  ],
  SUPERTREND: [
  ...
  ICHIMOKU_CLOUD: [
    ...
    { key: 'output_line', label: 'Output Line', type: 'select', options: ['TENKAN', 'KIJUN', 'SENKOU_A', 'SENKOU_B', 'CHIKOU'], default: 'TENKAN' }
  ],
  CONSTANT: [{ key: 'value', label: 'Value', default: 0 }],

  CANDLE_PATTERN: [
    { key: 'pattern', label: 'Pattern', type: 'enum', enumKey: 'CandlePatternType', default: 'DOJI' }
  ],
  CANDLE_BODY_SIZE: [
    { key: 'mode', label: 'Mode', type: 'select', options: ['POINTS', 'PERCENTAGE'], default: 'POINTS' }
  ],

  MATH_EXPRESSION: [
    { key: 'expression', label: 'Math Formula', type: 'math', default: { expression: '', variables: {} } }
  ],
  VWAP: [
    { key: 'anchor', label: 'Anchor', type: 'select', options: ['D', 'W', 'M'], default: 'D' }
  ],
  OBV: [],
  PIVOT_POINT: [],
  TIME_DECAY: [],
...
export default function OperandSelector({ 
  ...
  const paramConfig = value && value !== 'CONSTANT' && value !== 'MATH_EXPRESSION'
    ? [...baseParamConfig, { key: 'shift', label: 'Shift (Candles)', type: 'number', default: 0 }]
    : baseParamConfig;
    ...
    <div className="flex items-center gap-1">
      ...
      <div className="flex items-center bg-black/20 rounded-lg p-0.5 border border-white/[0.05]">
        <Select value={value || ''} onValueChange={onChangeType}>
          <SelectTrigger className={`w-auto min-w-[120px] bg-transparent border-none hover:bg-white/5 text-xs h-7 px-2.5 shadow-none focus:ring-0 ${value ? 'text-indigo-300' : 'text-gray-500'} font-semibold`}>
            ...
            {Object.entries(groupedOptions).map(([groupName, groupOptions]) => (
              groupOptions.length > 0 && (
                <SelectGroup key={groupName}>
                  <SelectLabel className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{groupName}</SelectLabel>
      ...
      {value && paramConfig.length > 0 && (
        paramConfig.some(p => p.type === 'math') ? (
          <Dialog open={popoverOpen} onOpenChange={setPopoverOpen}>
            <DialogTrigger asChild>
              <button className="flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold tracking-wider rounded-md bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-colors border border-indigo-500/20 cursor-pointer max-w-[200px] truncate">
                {value === 'CONSTANT' ? formatParams() : <span className="truncate">{currentOption?.label} {formatParams()}</span>}
            ...
            <DialogContent className="w-[500px] max-w-3xl bg-[#0f1423] border border-gray-800 shadow-2xl p-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-gray-800">
                  <h4 className="text-xs font-semibold text-gray-300">Build Math Expression</h4>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {paramConfig.map(param => (
                    <div key={param.key} className="col-span-2 w-full pt-1">
                      <MathExpressionBuilder 
                        value={params?.[param.key] || { expression: '', variables: {} }}
                        onChange={(val) => handleParamChange(param.key, val, true)}
                      />
                    </div>
                  ))}
                </div>
          ...
          <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
            ...
            <PopoverContent className="w-64 p-3 bg-[#0f1423] border border-gray-800 shadow-xl" sideOffset={5}>
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-gray-800">
                  <h4 className="text-xs font-semibold text-gray-300">Configure Parameters</h4>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {paramConfig.map(param => (
                    <div key={param.key} className={(param.type === 'source' && PARAM_CONFIG[params?.[param.key]]) ? "col-span-2 space-y-1" : "space-y-1"}>
                      <label className="text-[10px] text-gray-400 font-medium">{param.label}</label>
                      {param.type === 'source' ? (
                         <Select 
                           value={params?.[param.key] ?? param.default}
                           onValueChange={(v) => handleParamChange(param.key, v, true)}
                         >
                           ...
                           <SelectContent>
                             <SelectGroup>
                               ...
                               {['close', 'open', 'high', 'low', 'hl2', 'hlc3', 'ohlc4'].map(src => (
                                  <SelectItem key={src} value={src}>{src.toUpperCase()}</SelectItem>
                               ))}
                             </SelectGroup>
                             <SelectGroup>
                               <SelectLabel>Indicators</SelectLabel>
                               {Object.keys(PARAM_CONFIG).filter(k => k !== 'CONSTANT').map(ind => (
                        ...
                        <Input
                          type={param.key === 'option_type' ? "text" : "number"}
                          value={params?.[param.key] ?? param.default}
                          onChange={(e) => handleParamChange(param.key, e.target.value, param.key === 'option_type')}
                          className="h-7 text-xs bg-black/30 border-gray-800 text-gray-200"
                        />
                      )}
  
                      {param.type === 'source' && PARAM_CONFIG[params?.[param.key]] && (
                        <div className="mt-2 p-2 border border-gray-800 rounded bg-black/20">
                            <span className="text-[10px] text-indigo-400 font-bold mb-1 block">Source ({params?.[param.key]}) Settings</span>
                            <div className="grid grid-cols-2 gap-2">
                               {PARAM_CONFIG[params?.[param.key]].filter(p => p.type !== 'source').map(sp => (
                                   <div key={`source_${sp.key}`} className="space-y-1">
                                      <label className="text-[10px] text-gray-400 font-medium">{sp.label}</label>
                                      <Input
                                        type={sp.key === 'option_type' ? "text" : "number"}
                                        value={params?.source_params?.[sp.key] ?? sp.default}
                                        onChange={(e) => {
                                           const val = sp.key === 'option_type' ? e.target.value : (e.target.value === '' ? null : parseFloat(e.target.value));
                                           const currentSourceParams = params?.source_params || {};
                                           onChangeParams({ ...params, source_params: { ...currentSourceParams, [sp.key]: val } });
                                        }}
                                        className="h-7 text-xs bg-black/30 border-gray-800 text-gray-200"
                                      />
                                   </div>
      ...
      {!hideTimeframe && value && value !== 'CONSTANT' && !value.includes('POSITION') && value !== 'ENTRY_PRICE' && (
