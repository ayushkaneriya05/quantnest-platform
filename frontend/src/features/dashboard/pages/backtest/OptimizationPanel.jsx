...
import { 
  Play, Settings, Plus, Trash2, Zap, BarChart2
} from 'lucide-react';
...
export default function OptimizationPanel() {
  ...
  const fetchData = async () => {
    try {
      const [stratData, optData] = await Promise.all([
        strategyApi.getAll(),
  ...
  const handleRemoveParam = (idx) => {
    setFormData({
      ...formData,
      parameters_to_optimize: formData.parameters_to_optimize.filter((_, i) => i !== idx)
    });
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(v) => setFormData({ ...formData, optimization_metric: v })}
                >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="text-gray-400">Parameters to Optimize</Label>
                  <Button type="button" size="sm" variant="outline" onClick={handleAddParam} className="border-gray-700">
                  ...
                  <div key={idx} className="grid grid-cols-5 gap-3 items-end">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="number"
                        value={param.step}
                        onChange={(e) => handleParamChange(idx, 'step', parseFloat(e.target.value))}
                        className="bg-gray-800 border-gray-700 text-white"
                      />
                    </div>
                    <Button 
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {opt.strategy_name} • {formatDate(opt.start_date)} - {formatDate(opt.end_date)}
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="text-sm text-gray-400">Progress</p>
                      <p className="text-white font-medium">{opt.progress_pct || 0}%</p>
                    </div>
                    <Badge className={opt.status === 'COMPLETED' ? 'bg-green-600' : 'bg-blue-600'}>
                      {opt.status}
                    </Badge>
                  </div>
                </div>
