...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (paperRes.status === "fulfilled") setPaperPortfolio(paperRes.value.data);
  ...
  const paperValue = parseFloat(paperPortfolio?.invested_value || 0) + parseFloat(paperPortfolio?.unrealized_pnl || 0) + parseFloat(paperPortfolio?.available_cash || 0);
  const liveValue = parseFloat(liveSummary?.total_allocated || 0) + parseFloat(liveSummary?.unrealized_pnl || 0);
  
  // Formatter for currency
  const formatCurrency = (val) => new Intl.NumberFormat('hi-IN', { style: 'currency', currency: 'INR', minimumFractionDigits: 2 }).format(val || 0);
    ...
    <div className="container-padding py-6 lg:py-8 space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <Card className="bg-gray-900/40 backdrop-blur-md border-gray-800/60 shadow-lg hover:shadow-indigo-500/10 transition-all duration-300 hover:-translate-y-1">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-100">{formatCurrency(paperValue)}</div>
            <p className={`text-xs mt-1 ${parseFloat(paperPortfolio?.unrealized_pnl || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              {parseFloat(paperPortfolio?.unrealized_pnl || 0) >= 0 ? '+' : ''}{formatCurrency(paperPortfolio?.unrealized_pnl || 0)} Open PnL
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-900/40 backdrop-blur-md border-gray-800/60 shadow-lg hover:shadow-purple-500/10 transition-all duration-300 hover:-translate-y-1">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-100">{formatCurrency(liveValue)}</div>
            <p className="text-xs text-slate-400 mt-1">Active in {liveSummary?.active_sessions_count || 0} strategies</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-900/40 backdrop-blur-md border-gray-800/60 shadow-lg hover:shadow-indigo-500/10 transition-all duration-300 hover:-translate-y-1">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-slate-100">
              {report.total_trades ? ((report.winning_trades / report.total_trades) * 100).toFixed(1) : "0.0"}%
            </div>
            <p className="text-xs text-slate-400 mt-1">{report.winning_trades || 0}W / {report.losing_trades || 0}L</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-900/40 backdrop-blur-md border-gray-800/60 shadow-lg hover:shadow-cyan-500/10 transition-all duration-300 hover:-translate-y-1">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Zap className={`h-4 w-4 ${parseFloat(report.realized_pnl) >= 0 ? 'text-emerald-400' : 'text-red-400'}`} />
          </CardHeader>
          <CardContent>
            <div className={`text-2xl font-bold ${parseFloat(report.realized_pnl) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              {parseFloat(report.realized_pnl) > 0 ? '+' : ''}{formatCurrency(report.realized_pnl || 0)}
            </div>
            <p className="text-xs text-slate-400 mt-1">Across {report.total_trades || 0} trades today</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <CardContent>
            <div className="space-y-3">
              {insights.length > 0 ? insights.map((insight) => (
                <div key={insight.id} className="flex flex-col p-4 rounded-lg bg-gray-800/40 border border-gray-700/30 hover:bg-gray-800/60 transition-all">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-slate-200 font-medium">{insight.title}</span>
                ...
                <div className="p-4 text-center text-slate-500">No recent insights generated. Check your Trade Journal.</div>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-900/40 backdrop-blur-md border-gray-800/60 shadow-lg hover:shadow-card-hover transition-all duration-300">
          ...
          <CardContent>
            <div className="space-y-3">
              {snapshots.length > 0 ? [...new Map(snapshots.map(item => [item.strategy?.id, item])).values()].slice(0, 4).map(snap => (
                <div key={snap.id} className="flex justify-between items-center p-3 rounded-lg bg-gray-800/40 border border-gray-700/30 hover:bg-gray-800/60 transition-all">
                  <div>
                    <p className="text-slate-200 font-medium">{snap.strategy?.name || "Unnamed Strategy"}</p>
                    <p className="text-xs text-slate-400">Win Rate: {parseFloat(snap.win_rate || 0).toFixed(1)}%</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-sm font-bold ${parseFloat(snap.daily_pnl) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                      {parseFloat(snap.daily_pnl) > 0 ? '+' : ''}{formatCurrency(snap.daily_pnl)}
                    </p>
                    <p className="text-xs text-slate-400">{snap.trades_count} trades today</p>
                  </div>
                </div>
              )) : (
                <div className="p-4 text-center text-slate-500">No active strategies running today.</div>
              )}
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-4">
        <Card className="bg-gradient-to-br from-indigo-500/10 to-purple-600/10 border-indigo-500/30 hover:border-indigo-400/50 transition-all duration-300 hover:-translate-y-1">
          <CardHeader>
            ...
            <CardDescription className="text-slate-300">
              Create, configure, and backtest technical trading strategies using our intuitive rule builder.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button onClick={() => navigate('/dashboard/strategy/create')} className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 border-0">
              Build Strategy
        ...
        <Card className="bg-gradient-to-br from-emerald-500/10 to-teal-600/10 border-emerald-500/30 hover:border-emerald-400/50 transition-all duration-300 hover:-translate-y-1">
          <CardHeader>
            ...
            <CardDescription className="text-slate-300">
              Execute your strategies in a simulated environment using real-time market data.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button onClick={() => navigate('/dashboard/paper')} className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 border-0">
              Start Trading
              <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </CardContent>
        ...
        <Card className="bg-gradient-to-br from-orange-500/10 to-red-600/10 border-orange-500/30 hover:border-orange-400/50 transition-all duration-300 hover:-translate-y-1">
          <CardHeader>
            ...
            <CardDescription className="text-slate-300">
              Automated recommendations based on strategy health scoring, market regime detection, and overfit analysis.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button onClick={() => navigate('/dashboard/ai/advisor')} className="w-full bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 border-0">
              View Advisor
              <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </CardContent>
