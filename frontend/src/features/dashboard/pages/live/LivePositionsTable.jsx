      ...
      <Table>
        <TableHeader className="bg-gray-950/40">
          <TableRow className="border-gray-800 hover:bg-transparent">
            ...
            <TableHead className="text-gray-500 font-medium text-right">Avg Price</TableHead>
            <TableHead className="text-gray-500 font-medium text-right">LTP</TableHead>
            <TableHead className="text-gray-500 font-medium text-right">Day P&L</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {positions.map((pos) => {
            const isProfit = Number(pos.day_pnl || 0) >= 0;
            return (
              <TableRow key={pos.id} className="border-gray-800 hover:bg-white/5 transition-colors">
                ...
                <TableCell>
                  <Badge className={pos.side === 'BUY' ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/20' : 'bg-rose-500/10 text-rose-300 border-rose-500/20'}>
                    {pos.side}
                  </Badge>
                </TableCell>
                ...
                <TableCell className={`text-right font-mono font-bold ${isProfit ? 'text-emerald-300' : 'text-rose-300'}`}>
                  <div className="flex items-center justify-end gap-1">
                    {isProfit ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                    Rs {formatNumber(pos.day_pnl)}
                  </div>
                </TableCell>
              </TableRow>
            );
