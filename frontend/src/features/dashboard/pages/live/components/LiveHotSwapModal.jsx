...
export default function LiveHotSwapModal({
    ...
    if (open && session?.strategy) {
      const fetchVersions = async () => {
        try {
          ...
          if (session?.allocation?.version_id) {
            setSelectedVersion(String(session.allocation.version_id));
          } else {
            setSelectedVersion("");
          }
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedVersion || !session?.allocation?.id) return;
    try {
      setLoading(true);
      await liveTradingApi.deployVersion(session.allocation.id, {
        version_id: selectedVersion,
      });
      notify.success("Strategy version hot-swapped successfully");
      onSuccess?.();
      onOpenChange(false);
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-gray-950 border-gray-800 text-gray-100">
        <DialogHeader>
          <DialogTitle>Hot-Swap Strategy Version</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          <Alert className="bg-amber-900/20 border-amber-800/50 text-amber-300 mb-2">
            ...
            <AlertDescription className="text-xs ml-2">
              <strong>Warning:</strong> Hot-swapping a strategy while it has active open positions may lead to unexpected behavior if the new version expects different state. It is recommended to flatten positions or pause the session before hot-swapping.
            </AlertDescription>
          </Alert>
            ...
            <div className="flex justify-between items-center">
              ...
              {session?.allocation?.version_number && (
                <span className="text-xs text-gray-400">
                  Current: <strong className="text-white">v{session.allocation.version_number}</strong>
                </span>
              )}
            </div>
            {fetchingVersions ? (
